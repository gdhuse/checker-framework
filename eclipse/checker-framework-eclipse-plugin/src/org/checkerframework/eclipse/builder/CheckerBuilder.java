...
public class CheckerBuilder extends IncrementalProjectBuilder {
	...
	private void incrementalBuild(IResourceDelta delta) {
		...
		runWorker(visitor.getBuildFiles(), CheckerManager.getSelectedClasses());
	}

	private void fullBuild() throws CoreException {
		IJavaProject project = JavaCore.create(getProject());
		Set<IJavaElement> sourceElements = ResourceUtils
				.javaElementsOf(project);

		runWorker(sourceElements, CheckerManager.getSelectedClasses());
	}

	private void runWorker(Set<IJavaElement> sourceElements,
			List<String> checkerNames) {
		Job checkerJob = new CheckerWorker(sourceElements, PluginUtil.join(",",
				checkerNames), true);
